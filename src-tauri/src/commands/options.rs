...
pub fn get_option_contracts(
    ...
    // Pair contracts: group by option_symbol
    ...
    for (symbol, recs) in &grouped {
        // Find the SELL+O record (open)
        let open_rec = recs.iter().find(|r| r.action == "SELL" && r.code == "O");
        // Find the BUY close record (C;Ep or A;C)
        let close_rec = recs
            .iter()
    ...
    // Sort: active first, then by underlying, then by expiry_date
